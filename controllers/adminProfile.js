...
// async function getSalonByServiceNameOrLocation(req, res) {
//   try {
//     const { latitude, longitude, serviceName, locationName, sortBy } = req.body;
...
//     // Base query without sort
//     let query = adminProfileModel.find(filter);
...
//       query = adminProfileModel.find(filter);
//       // Note: MongoDB automatically sorts $near results by distance
//     }

//     // If no $near (no lat/lon), you can apply sort manually
//     if (!latitude || !longitude) {
//       if (sortBy === "rating") {
//         query = query.sort({ avgRating: -1 }); // Highest rating first
//       } else {
//         query = query.sort({ createdAt: -1 }); // Newest salons
//       }
//     }
